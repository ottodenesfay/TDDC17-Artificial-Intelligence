...
class MyAgentProgram implements AgentProgram {
	...
	public void find_node(int goal) {
		...
		while (!frontier.isEmpty()) {
			...
			/* Generate neighbors, disregard if a wall or already exist in nodes_with_parent */
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
					Point neighbor_node = new Point((int)next_node.getX() + x, (int)next_node.getY() + y);
					if (Math.abs(x) != Math.abs(y) 
						&& state.world[(int)neighbor_node.getX()][(int)neighbor_node.getY()] != state.WALL
						&& !nodes_with_parent.containsKey(neighbor_node) && !frontier.contains(neighbor_node)) {
						nodes_with_parent.put(neighbor_node, next_node);
						frontier.offer(neighbor_node);
					}
				}
			}
		}
	...
	/* Simulates the vacuum moving from the current pos to the node found in goal_node.
	...
	public Action execute(Percept percept) {
	    ...
	    /* If the goal node (found through find_node) is reached */
