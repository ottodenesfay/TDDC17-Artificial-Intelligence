...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while (!frontier.isEmpty()) {
			SearchNode next_node = frontier.removeFirst();
			if (p.isGoalState(next_node.getState())) { // if the node is our goal destination
				path = next_node.getPathFromRoot(); // get the path from root to the node
				return path;
			}
			explored.add(next_node); 
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(next_node.getState()); // get the nodes "childs" grid positions
			for (GridPos tmp : childStates) { // iterate over the child nodes
				SearchNode child = new SearchNode(tmp, next_node);
				if (!explored.contains(child) && !frontier.contains(child)) {    // if the node has not been explored yet and is not already in frontier
					System.out.println(frontier.size());
					if (insertFront)
						frontier.addNodeToBack(child); // add the child node to the queue in a BFS type fashion
					else
						frontier.addNodeToFront(child); // add the child node to the queue in a DFS type fashion
				}
			}
		}
		return path;
