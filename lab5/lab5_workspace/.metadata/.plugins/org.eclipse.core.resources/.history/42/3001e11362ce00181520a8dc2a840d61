...
public class QLearningController extends Controller {
	...
	void performAction(int action) {
		resetRockets();
		/* Execute an action based on the value of "action" */
		switch (action) {
		case 1: rightEngine.setBursting(true);
				break;
		case 2: middleEngine.setBursting(true);
				break;
		case 3: leftEngine.setBursting(true);
				break;
		}
	}
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				/* Learning rate based on nr of times prev_stateaction has been tested */
				double tmp_alpha = alpha(Ntable.get(prev_stateaction));
				
				/* Update-function for Qtable */
				double value = Qtable.get(prev_stateaction) + (tmp_alpha * 
